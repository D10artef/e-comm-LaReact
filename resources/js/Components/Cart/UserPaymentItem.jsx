...
const UserPaymentItem = ({payment, active, selectPayement}) => {
  ...
  return (
    <div className={divClass}>
      <input type="radio" name="user_payement" value={payment.id} onChange={selectPayement}/>
      <Icon name='credit-card' className='w-6 h-6 text-gray-500'/>
      <div className="flex flex-col">
        {/* <h1>{payment.provider_name}</h1> */}
        {
          payment.provider?.name ? <h1>{payment.provider.name}</h1> : <h1>TOjo</h1>
        }
